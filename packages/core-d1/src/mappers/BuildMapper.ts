...
export class BuildMapper {
  static fromDTO(dto: D1BuildDTO): Build {
    return {
      ...
      name: dto.name,
      stage: BuildMapper.mapStageFromDB(dto.stage),
      status: dto.status ? BuildMapper.mapStatusFromDB(dto.status) : undefined,
  ...
  static toDTO(build: Build): D1BuildDTO {
    return {
      ...
      name: build.name,
      stage: BuildMapper.mapStageToDB(build.stage),
      status: build.status == null ? 4 : BuildMapper.mapStatusToDB(build.status),
