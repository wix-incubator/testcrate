...
describe('ProjectController integration', () => {
  ...
  test('putProject creates a new project', async () => {
    ...
    const getReq: GetProjectRequest = { id: 'p1' };
  ...
  test('putProject upserts when id already exists', async () => {
    ...
    await controller.putProject(request);
    const got = await controller.getProject({ id: 'p1' });
  ...
  test('patchProject updates existing and bumps categories revision', async () => {
    // seed
    await controller.putProject({ id: 'p2', payload: {
      name: 'P2',
      description: 'd',
      categories: [],
    } });
    const before = await controller.getProject({ id: 'p2' });
    ...
    const stored = await controller.getProject({ id: 'p2' });
    expect(stored?.updated?.ts).toBeGreaterThan(0);
    expect(stored?.created?.ts).toBeGreaterThan(0);
  });

  test('patchProject throws when not found', async () => {
    const patchReq: PatchProjectRequest = { id: 'missing', payload: { name: 'x' } };
  ...
  test('deleteProject returns true and removes project', async () => {
    await controller.putProject({ id: 'p3', payload: {
      name: 'P3',
      description: 'd',
      categories: [],
    } });
  ...
  test('deleteProject returns false for missing project', async () => {
    const deleted = await controller.deleteProject({ id: 'nope' } satisfies DeleteProjectRequest);
  ...
  test('listProjects returns paginated items', async () => {
    await controller.putProject({ id: 'a', payload: {
      name: 'A',
      description: 'd',
      categories: [],
    } });
    await controller.putProject({ id: 'b', payload: {
      name: 'B',
      description: 'd',
      categories: [],
    } });
    await controller.putProject({ id: 'c', payload: {
      name: 'C',
      description: 'd',
      categories: [],
    } });
