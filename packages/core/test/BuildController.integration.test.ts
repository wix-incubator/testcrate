...
import type { BuildController } from '@core/controllers';
...
describe('BuildController integration', () => {
  ...
  test('putBuild creates or replaces a build', async () => {
    const req: PutBuildRequest = {
      projectId: 'p1',
      buildId: 'b1',
      payload: {
        historyId: 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
        stage: 'scheduled' as BuildStage,
      } as Build,
    };

    const created = await controller.putBuild(req);
  ...
  test('patchBuild updates existing build', async () => {
    await controller.putBuild({ projectId: 'p1', buildId: 'b2', payload: { historyId: 'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', stage: 'scheduled' as BuildStage } });
  ...
  test('deleteBuild returns true when present, false otherwise', async () => {
    await controller.putBuild({ projectId: 'p1', buildId: 'b3', payload: { historyId: 'cccccccccccccccccccccccccccccccc', stage: 'scheduled' as BuildStage } });
  ...
  test('listBuilds returns items and respects filters', async () => {
    await controller.putBuild({ projectId: 'pA', buildId: '1', payload: { historyId: '11111111111111111111111111111111', stage: 'scheduled' as BuildStage } });
    await controller.putBuild({ projectId: 'pA', buildId: '2', payload: { historyId: '22222222222222222222222222222222', stage: 'finished' as BuildStage } });
    await controller.putBuild({ projectId: 'pB', buildId: '3', payload: { historyId: '33333333333333333333333333333333', stage: 'scheduled' as BuildStage } });
