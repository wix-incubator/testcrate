...
router.all(
  '*',
  async (request: AppRequest, env: Env, _ctx: ExecutionContext) => {
    // Get or create the composition root singleton
    request.compositionRoot = await getCompositionRootFromEnv(env, request);
  },
);
...
export default {
  async fetch(
    ...
    try {
      ...
      const compositionRoot = await getCompositionRootFromEnv(env, request);
